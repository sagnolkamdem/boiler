...
export class AddProofService {
    ...
    async update(proofId: string, addProofInput: AddProofInput): Promise<AddProofOutPut> {

        if (addProofInput.score.length > 3) {
            return { 
                message: "You cannot justify more than 3 scores with the system. Please see administration!", 
                statusCode: 400
            }
        }
