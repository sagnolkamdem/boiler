...
export class CreatePayStubService {
    ...
    async create(createPayStubInput: CreatePayStubInput): Promise<CreatePayStubOutput> {
        ...
        try {
            ...
            if (user && !oldPayStub) {
                ...
                let message: string;
                !user ? message = "User not found" : message = "Pay stub of that month already exists";
                return {
                    message: message,
                    statusCode: 400,
                    payStub: null
